...
public record ApplianceDTO(
    @Schema(description = "Name for identifying an appliance in an address")
    @NotBlank(message = "cannot be null or empty")
    @Size(min = 1, max = 150, message = "size must be between {min} and {max}")
    String name,

    @Schema(description = "Brand name that helps people recognize and identify an appliance manufacturer")
    @NotBlank(message = "cannot be null or empty")
    @Size(min = 1, max = 150, message = "size must be between {min} and {max}")
    String brand,

    @Schema(description = "A combination of numbers and letters to identify an appliance")
    @NotBlank(message = "cannot be null or empty")
    @Size(min = 1, max = 150, message = "size must be between {min} and {max}")
    String model,

    @Schema(description = "Electric tension or potential difference unit from an appliance")
    @NotNull(message = "cannot be null or empty")
    @Size(min = 4, max = 4, message = "size must be {max}")
    Power power,

    @Schema(description = "UUID from an existent address")
    @NotBlank(message = "cannot be null or empty")
